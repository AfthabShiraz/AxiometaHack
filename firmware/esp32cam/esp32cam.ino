// ESP32-CAM (AI-Thinker) WiFi MJPEG streamer + rover serial bridge.
//
// Video: joins the network in wifi_secrets.h and serves live video at
...
//
// Bridge (makes the robot wireless — plan.md open question 2): listens on
// UDP :5010 for text commands ("M,<l>,<r>\n", "S\n") and forwards them
// verbatim over UART2 to the Arduino; Arduino replies (A/H/W lines) are
// sent back to the most recent commander. The bridge never repeats or
// synthesizes commands — if the hub goes silent the Arduino watchdog
// stops the motors (FR-15).
//
// Wiring (no SD card, so GPIO 14/15 are free):
//   GPIO 14 (TX2) -> Arduino RX (pin 0)          [3.3V, direct]
//   Arduino TX (pin 1) -> divider 1k/2k -> GPIO 15 (RX2)  [5V -> 3.3V!]
//   common ground (already shared via L298N)
...
const uint16_t BRIDGE_PORT = 5010;
const int BRIDGE_TX = 14;  // -> Arduino RX
const int BRIDGE_RX = 15;  // <- Arduino TX via divider
...
void setup() {
  ...
  Serial2.begin(115200, SERIAL_8N1, BRIDGE_RX, BRIDGE_TX);
  bridgeUdp.begin(BRIDGE_PORT);
}

// UDP command in -> UART out; UART reply lines -> UDP back to commander.
static void bridge_poll() {
  while (true) {
    int len = bridgeUdp.parsePacket();
    if (len <= 0) break;
    char buf[48];
    int n = bridgeUdp.read(buf, sizeof(buf) - 1);
    if (n <= 0) continue;
    buf[n] = '\0';
    commanderIp = bridgeUdp.remoteIP();
    commanderPort = bridgeUdp.remotePort();
  ...
  while (Serial2.available() > 0) {
    char c = Serial2.read();
    if (c == '\n' || c == '\r') {
      if (arduinoLineLen > 0 && commanderPort != 0) {
        arduinoLine[arduinoLineLen] = '\n';
        bridgeUdp.beginPacket(commanderIp, commanderPort);
        bridgeUdp.write((const uint8_t *)arduinoLine, arduinoLineLen + 1);
        bridgeUdp.endPacket();
      }
      arduinoLineLen = 0;
    } else if (arduinoLineLen < sizeof(arduinoLine) - 1) {
      arduinoLine[arduinoLineLen++] = c;
    }
  }
}
...
void loop() {
  ...
  if (!connected && now - lastAttempt > 20000) {
  ...
  if (now - lastStatus > 5000) {
    lastStatus = now;
    Serial.printf("bridge last_cmd_age_ms=%ld\n",
                  commanderPort ? (long)(now - lastCommandAt) : -1);
  }
  wasConnected = connected;
  delay(2);  // keep bridge latency low; camera serving runs in httpd task
}
